...
def getFotMobSeasonFixture(league,seasons): # Example [2016,2017] or "23_24"
    ...
    if "list" in str(type(seasons)):
        ...
        fixture_urls = [FOTMOB_URL['fixture'].format(FOTMOB_LEAGUE[league],season_name_str)]
    all_fixtures = []
...
def combine_dataframes(league,WS_init,FB_init,FM_init,team_alias):
    WS = WS_init
    FB = FB_init
    FM = FM_init
    print(WS.shape)
    ...
    try:
        for x in WS.index:
            ...
            FM_row = FM[(FM['home_team'].isin(home_team_alias)) & (FM['away_team'].isin(away_team_alias)) & (FM['season'] == row_season) & (FM['date'] == row_date)]
            FB_row = FB[(FB['home_team'].isin(home_team_alias)) & (FB['away_team'].isin(away_team_alias)) & (FB['season'] == row_season) & (FB['date'] == row_date)]
            ...
            if FB_row.empty or FM_row.empty or WS_row.empty:
                ...
                if not FB_row.empty:
                    ...
                    if "Cancelled" in str(FB_row.iloc[0]['notes']):
                        continue
                if WS.loc[x]['date'] > (datetime.now()+timedelta(days=2)).date:
                    print(f"Match Yet to happen on '{WS.loc[x]['date']}', today is '{(datetime.now()+timedelta(days=2)).date}' ")
                    continue
                return msg
            else:
...
def merge_and_save_df(league,WS,FB,FM,team_aliases,save=False):
    print("Merging Fetched DataFrames ... ")
    merged_df = combine_dataframes(league,WS,FB,FM,team_aliases)
    current_date = datetime.now().strftime("%d-%m-%Y")
    merged_df['date'] = pd.to_datetime(merged_df['date'])
    merged_df = merged_df[merged_df['date'] <= current_date]
    if save:
